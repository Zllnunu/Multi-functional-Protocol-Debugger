...
"""
fpga_contrl.py
把“串口控制 + SPI/I2C/UART/PWM/SEQ/原始发送 + 实时绘图”整合为一个可嵌入的 QWidget 页面。
（以太网采集模块保留在 osc_ui.py 中，本文件不包含以太网相关逻辑）
"""
...
# ------------------------------------------------------------------
# 3) 主控页：把所有“控制模块”作为子页 (Tabs)，便于在外层翻页系统中嵌入
# ------------------------------------------------------------------
class FpgaControlWidget(QWidget):
    ...
    def send_protocol_command(self, proto: str):
        # 这里给出统一的示例命令风格；实际你可以改成和固件一致的协议帧
        if proto == 'IIC':
            p = self.controls['IIC']
